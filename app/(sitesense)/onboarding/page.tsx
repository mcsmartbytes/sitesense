    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="py-8 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-3xl font-bold text-white mb-2">Welcome to SiteSense</h1>
      ...
      <div className="max-w-2xl mx-auto px-4 mb-8">
        <div className="flex items-center justify-center gap-4">
          {['select', 'customize', 'complete'].map((s, i) => (
            <div key={s} className="flex items-center">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold transition-colors ${
                  step === s
                    ? 'bg-blue-600 text-white'
                    : ['customize', 'complete'].indexOf(step) > ['select', 'customize', 'complete'].indexOf(s as Step)
              ...
              {i < 2 && (
                <div className={`w-16 h-1 mx-2 ${
                  ['customize', 'complete'].indexOf(step) > i ? 'bg-green-600' : 'bg-slate-700'
                }`} />
              )}
            </div>
          ))}
        </div>
        <div className="flex justify-between mt-2 text-sm text-slate-400 px-4">
      ...
      <div className="max-w-4xl mx-auto px-4 pb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {profiles.map((profile) => (
                <button
                  ...
                  disabled={saving}
                  className="bg-slate-800/50 hover:bg-slate-700/50 border border-slate-700 hover:border-slate-600 rounded-xl p-6 text-left transition-all group"
                >
          ...
          <div>
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-semibold text-white">
                Customize for {selectedProfile.name}
              </h2>
              <p className="text-slate-400 mt-2">
                Personalize your terminology and enabled features
              </p>
            </div>
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {enabledModules.map((module) => (
                    <label
                      key={module}
                      className="flex items-center gap-3 p-3 bg-slate-900/50 rounded-lg cursor-pointer hover:bg-slate-900"
                    >
                      <input
                        type="checkbox"
        ...
        {step === 'complete' && (
          <div className="text-center py-12">
            <div className="inline-flex p-6 bg-green-600/20 rounded-full mb-6">
